...
<details>
    ...
    이번 프로젝트에서 ERD를 설계하며 가장 많이 고민했던 부분은 사용자가 구매한 커리큘럼을 어떻게 학습 캘린더에 반영할 것인가에 대한 구조였습니다.

    초기에는 사용자가 커리큘럼을 구매하면,  
    판매자의 일정 데이터를 전부 복사한 뒤 `content` 컬럼을 추가해 사용자가 수정할 수 있는 방식으로 설계하였습니다.  
    
    이 방식은 사용자에게 높은 자유도를 제공한다는 장점이 있었지만,  
    - 데이터 중복  
    - 커리큘럼 버전 불일치  
    - 유지보수의 어려움  
    등의 문제를 유발하였습니다.
    
    이에 따라 설계를 변경하여, 커리큘럼은 하나의 원본 데이터만 유지하고,  
    사용자의 캘린더에서는 이를 참조하는 방식으로 연결하여 보여주는 구조로 바꾸었습니다.
    
    이러한 경험을 통해 단순한 테이블 중심의 설계보다는,  
    사용자 경험, 비즈니스 흐름, 운영과 유지보수 측면까지 고려한 데이터 모델링의 중요성을 깊이 체감할 수 있었습니다.
    
    앞으로는 단순히 기능을 구현하는 데 그치지 않고,  
    실제 운영 시나리오와 사용자 행동 흐름까지 고려한 유연하고 확장 가능한 구조를 설계하는 데 초점을 맞출 것입니다.
</details>
<details>
    <summary>쿼리 작성하면서 느낀점</summary>
    
    이번 프로젝트에서는 단순한 조회나 조건 필터링을 넘어서,  
    반복적인 비즈니스 로직을 데이터베이스 레벨에서 효율적으로 처리하기 위해 프로시저 중심의 쿼리 구조를 설계하였습니다.
    
    프로시저 내부에서는 단순한 CRUD 연산뿐 아니라,  
    - 흐름 제어 (IF, CASE)  
    - 반복 처리 (LOOP, WHILE)  
    등을 활용하여 보다 복합적인 로직을 처리할 수 있도록 구성하였습니다.
    
    이 과정을 통해 단순 SQL 쿼리로는 해결하기 어려운 상황에서도,  
    프로시저가 제공하는 재사용성, 일관성 유지, 서버 부하 감소 등의 장점을 직접 체감할 수 있었습니다.
    
    또한 단순히 쿼리를 작성하는 수준을 넘어,  
    실제 서비스 흐름을 고려한 쿼리 구조 설계의 중요성을 배우고, 이를 책임지고 구현하는 사고력을 키울 수 있는 계기가 되었습니다.
    
    앞으로는 프로시저뿐만 아니라,  
    트리거, 이벤트, 커서 등 다양한 SQL 기능도  
    실제 상황에 맞게 적절히 활용할 수 있도록 지식과 경험을 더욱 넓혀가고자 합니다.
</details>
<details>
